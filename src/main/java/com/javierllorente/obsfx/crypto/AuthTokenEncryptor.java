 ...
 */
package com.javierllorente.obsfx.crypto;
...
public class AuthTokenEncryptor implements StringEncryptor {
    ...
    @Override
    public String encrypt(String token) throws IllegalBlockSizeException, BadPaddingException {
        Cipher cipher = getCipher(Cipher.ENCRYPT_MODE);
        byte[] encryptedData = cipher.doFinal(token.getBytes());
        String encryptedToken = Base64.getEncoder().encodeToString(encryptedData);      
        return encryptedToken;
    }

    @Override
    public String decrypt(String encryptedToken) throws IllegalBlockSizeException, BadPaddingException {
